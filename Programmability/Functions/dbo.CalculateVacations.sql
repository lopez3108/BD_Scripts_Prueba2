...
--Last edit by jt/25-07-2024 task 5878 Calculate the availlable vacations, in current cycle, previous cycles and VacationHoursAccumulated 
--Last edit by jt/29-05-2025 task 6398 Edit some commentaries and variables to clear the logic 
CREATE FUNCTION [dbo].[CalculateVacations] (@UserId INT,
@CycleDateVacation AS DATE,
...
BEGIN
  ...
  SELECT TOP 1
    @StartingDate =
    u1.StartingDate
   ,@CurrentCycleDateVacationTblUser = c.CycleDateVacation --This var help us to know the current cycle, this value is update in every cicly when pay the payroll
  FROM Cashiers c
  INNER JOIN Users u1
    ON c.UserId = u1.UserId
  WHERE c.UserId = @UserId

  --we need to know if the user has VacationHoursAccumulated in previous cycles (this var is true, when we are pay the payroll, because there we don't need to get the accumulated hours 
  ...
  BEGIN
    SELECT
      @vacationHoursAccumulatedPreviousCycle = (SELECT
          ISNULL(SUM(Hours), 0)
        FROM EmployeeVacationsAccumulated
        WHERE UserId = @UserId);
  END
  ...
  FROM TimeSheet
  WHERE UserId = @UserId
  AND ((CAST(TimeSheet.LoginDate AS DATE) >= @CycleDateVacation --The vacation hours only apply when the timesheet is greater or equal than the current cycle
  AND CAST(TimeSheet.LoginDate AS DATE) >= (SELECT TOP 1 --And The vacation hours Only apply when the cashier has a first  payroll, and is from  date of this payroll
  ...
  AND (CAST(TimeSheet.LoginDate AS DATE) <= DATEADD(DAY, -1, DATEADD(YEAR, 1, @CycleDateVacation))--And the vacation hours only apply to the finish the current cycle
  ...
  --Second we need to know if the user has VacationHoursAccumulated in previous cycles 
  IF (@NoAccumulatePreviousCycles = 0
    OR @NoAccumulatePreviousCycles IS NULL)--@NoAccumulatePreviousCycles only arraive NULL when we are searching pending hors from paryoll, because in the payroll only need to get the current cycle
  BEGIN
    SELECT
  ...
  SELECT
    @hourstaken = ISNULL((SELECT
        ISNULL(SUM(Hours), 0)
      FROM EmployeeVacations --Son las horas de vaciones que ha tomado
      WHERE UserId = @UserId
      AND (CAST(EmployeeVacations.CycleDateVacation AS DATE) >= @CycleDateVacation
      AND CAST(EmployeeVacations.CycleDateVacation AS DATE) <= @CycleDateVacation
      ))
    , 0)
  ...
  BEGIN
    ...
    SET @result =
    CASE
      WHEN (ISNULL(@hoursworked, 0) / 35) + ISNULL(@vacationHoursAccumulated, 0) <= 40 THEN ((ISNULL(@hoursworked, 0) / 35) + @vacationHoursAccumulated) - @hourstaken
      ELSE (40) - @hourstaken
    END;
  END
  ELSE
